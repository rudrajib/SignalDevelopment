...
P0=100*Nd;
L= 500; %Transmission block U=BcTc guess Bc=500kHz
d1=10;
d2=d1;
SINR1=10^(d1/10); %SNR 
SINR2=10^(d2/10);
...
lp_sum2=lp_sum2+1;
  cvx_begin gp
   ...
   variables pdup a1 pddn c1 a2
  
    minimize (Nd*pdup+a1^2+Nd*pddn+a2)
    subject to 
    %(K-1)+pdup^-1+(M/SINR1+1)*a1<=M/SINR1
    %(M-1)/M*(K-1)+c1<=(M-1)/SINR2
    %c1^-1*K*a1+c1^-1*pddn^-1+a1<=1
    ((M*pdup)^(-1)).*(a1*(K-1)+a2+a1)<= (1+a1)/SINR1
    K*a1+pddn^-1+(M-K)/SINR1*a1<=(M-K)/SINR2
    %(K-1)+pdup^-1<=M*(a1*SINR1)^-1+M*SINR1^-1+a1^-1
...
%EEZF_Optimal(1,lp_sum2)=(log2(1+x5)+log2(1+y5))/(10*log10(PPup*Np+PDup*Nd)+10*log10(PDdn*Nd)+10*log10(P_CP));

%cvx_begin gp
   %variables P_TC
   % minimize P_TC
   % subject to 
    % P_TC<=M*P_BS+P_SYN+K*P_UE
