%% This is the corrected version of final Prethesis version
%% Here I combine the total symbol power, make it logarithm and calculate enecgy efficiency
 ...
 %% MRC Energy Efficiency at Source to relay
        ...
        cvx_begin gp
            variables msps a1 gamma1
             %minimize K^2*lemda1*gamma1^(-eta1)*Nd*msps+K^2*a1^(-1)*lemda1*gamma1^(-eta1)
             minimize K^2*P*lemda1*gamma1^(-eta1)
            subject to
 ...
 %% MRT Energy Efficiency at relay to destination
        ...
        cvx_begin gp
            variables mdpd a2 c2 gamma2
             minimize K^2*lemda2*gamma2^(-eta2)*Nd*mdpd+K^2*a2^(-1)*lemda2*gamma2^(-eta2)
             %minimize K^2*P*lemda2*gamma2^(-eta2)
            subject to
    ...
    %EE_secdn(1,lp)=cap_dn_msec(1,lp)/(pp+pd);
     ...
     %% ZF Energy Efficiency at Source to relay
        ...
        cvx_begin gp
            ...
            %minimize K^2*lemda3*gamma3^(-eta3)*Nd*zsps+K^2*a3^(-1)*lemda3*gamma3^(-eta3)
             minimize P*K^2*lemda3*gamma3^(-eta3)
            subject to
...
zpps(1,lp)=((a3^(-1))-1)/Np;
ZSps(1,lp)=zsps;
Zps(1,lp)=10*log10(Np*zpps(1,lp)+Nd*ZSps(1,lp));
zf_ssec(1,lp)=gamma3;
...
%%%%%%%Total Secracy Capacity
    ...
    EE_zf_real(1,lp)=zf_up_rreal(1,lp)/P;
    EE_zf_sec(1,lp)=zf_up_msec(1,lp)/Zps(1,lp);
   % EE_sec(1,lp)=cap_up_msec(1,lp)/(pp+ps);
    ...
    %% ZF Energy Efficiency at relay to destination
        ...
        cvx_begin gp
            ...
            %minimize K^2*lemda4*gamma4^(-eta4)*Nd*zsps+K^2*a4^(-1)*lemda4*gamma4^(-eta4)
             minimize P*K^2*lemda4*gamma4^(-eta4)
            subject to
...
zppd(1,lp)=((a4^(-1))-1)/Np;
ZDpd(1,lp)=zdpd;
Zds(1,lp)=10*log10(Np*zppd(1,lp)+Nd*ZDpd(1,lp));
zf_secdn(1,lp)=gamma4;
zf_sd(1,lp)=10^(zf_secdn(1,lp)*0.1);

    %%%%%%%Total Secracy Capacity
    zf_dn_rreal(1,lp)=log2(1+zf_rd(1,lp));
    zf_dn_msec(1,lp)=log2(1+zf_sd(1,lp));
    EE_zf_realdn(1,lp)=zf_dn_rreal(1,lp)/P;
    EE_zf_secdn(1,lp)=zf_dn_msec(1,lp)/Zds(1,lp);
    %EE_secdn(1,lp)=cap_dn_msec(1,lp)/(pp+pd);
...
figure(1) 
 ...
 xlabel('Number of Antennas(M)');
 ylabel('Signal to Noise Ratio (SINRdB)');
 legend('MRCreal(S~R)','MRClower(S~R)','MRCreal(R~D)','MRClower(R~D)','Location','southeast');
